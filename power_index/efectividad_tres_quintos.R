...
info_votaciones_aprobadas <- info_votaciones %>%
  filter(Resultado=='APROBADO' & TotalSI>=93)
...
veces_convencional_all$n_iniciativas_aprobadas <- 77
...
veces_convencional_all_rechazos$n_iniciativas_rechazadas<- 85
...
# renombrar tresquintos

aprobaciones_tres_quintos <- veces_convencional_all

rechazos_tres_quintos <- veces_convencional_all_rechazos



write_rds(aprobaciones_tres_quintos,"temp/aprobaciones_tres_quintos.rds")

write_rds(rechazos_tres_quintos,"temp/rechazos_tres_quintos.rds")
